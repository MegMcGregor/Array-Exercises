...
export const getStudentByName = (fullName) => {
    ...
    return students.find(student => student.firstName.toLowerCase() + " " + student.lastName.toLowerCase() === fullName.toLowerCase())
...
export const getInstructorOfStudent = (studentId) => {
    ...
    let targetInstructor = instructors.find(instructor => targetStudent.instructorId === instructor.id)
...
export const getStudentWithMostLangs = () => {
 let numberOfLanguages = students.map(student => student.languages.length);
 let highestTotalLanguages = Math.max(...numberOfLanguages)
 return students.find(student => student.languages.length === highestTotalLanguages)
}
